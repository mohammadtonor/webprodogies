    ...
    <div className="flex justify-start gap-3 flex-wrap w-full">
      {gallery.length > 0 &&
        gallery.map((gal, key) =>
          validateURLString(gal).type === "IMAGE" ? (
            ...
            >
              <>
                <X className="absolute top-0 right-2 z-50 text-red-500" />
      ...
      {userid === groupUserid ? (
        <GlassModal
          ...
          trigger={
            <Card className="border-dashed border-themeGray mt-2 hover:bg-themeBlack bg-transparent cursor-pointer">
